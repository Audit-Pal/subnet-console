...
export default function TaskDetailPage() {
    ...
    const [selectedVuln, setSelectedVuln] = useState<string | null>("V-001");

    useEffect(() => {
        const loadTask = () => {
            const stored = typeof window !== 'undefined' ? JSON.parse(localStorage.getItem('tasks') || '[]') : [];
            const found = stored.find((t: any) => t.id === params.id);
            if (found && found.findings) {
                // FALLBACK: If code is missing (legacy tasks), use mock code
                if (!found.code) found.code = auditData.code;
                setAudit(found);
            } else {
                setAudit({ ...auditData, id: params.id as string });
            }
            setIsLoading(false);
        };
        const timer = setTimeout(loadTask, 400);
    ...
    const currentVuln = auditData.vulnerabilities.find(v => v.id === selectedVuln);
                    ...
                    <div className="w-full lg:w-[420px] xl:w-[480px] flex flex-col bg-zinc-950/50 min-h-0 shrink-0">
                        ...
                        <div className="p-8 border-b border-white/[0.08] relative overflow-hidden group">
                            ...
                            <div className="grid grid-cols-4 gap-2 mt-8">
                                {[
                                    { label: 'Crit', count: audit.findings?.critical, color: 'text-red-500', bg: 'bg-red-500/10' },
                                    { label: 'High', count: audit.findings?.high, color: 'text-orange-500', bg: 'bg-orange-500/10' },
                                    { label: 'Med', count: audit.findings?.medium, color: 'text-amber-500', bg: 'bg-amber-500/10' },
                                    { label: 'Low', count: audit.findings?.low, color: 'text-blue-500', bg: 'bg-blue-500/10' },
                                ].map((item) => (
                        ...
                        <div className="flex-1 flex flex-col min-h-0">
                            <div className="h-10 px-6 flex items-center justify-between border-b border-white/[0.08] bg-zinc-900/50 shrink-0">
                                ...
                                <span className="text-[10px] text-zinc-600 font-mono">{auditData.vulnerabilities.length} Found</span>
                            </div>
